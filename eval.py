...
if __name__ == "__main__":
    ...
    all_1 = np.array([])
    all_trans_1 = np.array([])
    all_rot_1 = np.array([])
    all_2 = np.array([])
    all_trans_2 = np.array([])
    all_rot_2 = np.array([])
        ...
        with torch.no_grad():
            for i, batch in enumerate(test_loader):
                ...
                if i < 2050 % 2:
                    ...
                    all_2 = np.concatenate([all_2, losses])
                    all_trans_2 = np.concatenate([all_trans_2, trans])
                    all_rot_2 = np.concatenate([all_rot_2, rot])
        print(all_1.mean())
        print(all_trans_1.mean())
        print(all_rot_1.mean())
        print(all_2.mean())
