...
void check() { std::cout << "ok" << std::endl; }
...
BOOST_PYTHON_MODULE(tartempion) {
  ...
  bp::def("check", &check);
  bp::def("backward_pass", &backward_pass);
  bp::def("forward_pass", &forward_pass);
  bp::def("check_qp", &test_qp);
  bp::class_<QP_pass_workspace>("QPworkspace", bp::init<>())
      .def("grad_p", &QP_pass_workspace::grad_p)
      .def("grad_A", &QP_pass_workspace::grad_A)
      .def("last_q", &QP_pass_workspace::get_last_q)
      .def("get_q", &QP_pass_workspace::Get_positions_)
      .def("set_L1", &QP_pass_workspace::set_L1_weight)
      .def("set_rot_w", &QP_pass_workspace::set_rot_weight)
      .def("set_q_reg", &QP_pass_workspace::set_q_reg)
      .def("set_lambda", &QP_pass_workspace::set_lambda)
      .def("set_tool_id", &QP_pass_workspace::set_tool_id)
      .def("set_bound", &QP_pass_workspace::set_bound)
      .def("grad_b", &QP_pass_workspace::grad_b);
}
