...
struct SE3InductiveBias {
  Eigen::Matrix<double, Eigen::Dynamic, 6> target_placement;
  Eigen::MatrixXd grad_out;
  std::vector<Eigen::MatrixXd> grad_propagation;
  Eigen::MatrixXd compute_T_target(
      const Eigen::MatrixXd &batched_q,
      const PINOCCHIO_ALIGNED_STD_VECTOR(pinocchio::SE3) & start_positions);
  Eigen::MatrixXd d_compute_T_target(const Eigen::MatrixXd &batched_grads);
};