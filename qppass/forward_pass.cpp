...
// void QP_pass_workspace::allocate(const pinocchio::Model &model, int
// batch_size,
//                                  int seq_len, int cost_dim, int eq_dim,
//                                  int num_thread) {
//   if (batch_size != batch_size_ || seq_len != seq_len_ ||
//       cost_dim != cost_dim_ || eq_dim != eq_dim_ || num_thread !=
//       num_thread_) {
...
//         Eigen::RowMajor>> A(A_ptr, eq_dim, 6);
...
//     jac.setZero();
//     pinocchio::computeFrameJacobian(model, data, q, tool_id,
//     pinocchio::LOCAL,
//                                     jac);
...
//     Eigen::MatrixXd &adj_diff = workspace.adj_diff[batch_id * seq_len +
//     time]; diff = current_placement.actInv(target_placement); adj =
//     current_placement.inverse().toActionMatrix(); workspace.target[batch_id *
//     seq_len + time] = target_lie; adj_diff = diff.toActionMatrixInverse();
//     Eigen::VectorXd err = pinocchio::log6(diff).toVector();

//     if (time > 0) {
//       workspace.workspace_.warm_start_x[batch_size * batch_id + time] =
//           workspace.workspace_.qp[batch_size * batch_id + time -
//           1]->results.x;
...
//          "You must set workspace's tool id.
...
//   workspace.allocate(model, batch_size, seq_len, cost_dim, eq_dim,
//   num_thread);
...
//                            int &cost_dim, int &tool_id, pinocchio::Data
//                            &data, const Eigen::Map<VectorXd> &q,
//                            Eigen::VectorXd &v, Eigen::VectorXd &a,
//                            Eigen::MatrixXd &v_partial_dq, Eigen::MatrixXd
//                            &v_partial_dv) {
...
//                                  pinocchio::Model &model, pinocchio::SE3
//                                  &diff, Eigen::VectorXd &rhs_grad, double
//                                  lambda, QP_pass_workspace &workspace, int
//                                  thread_id, int cost_dim, double dt) {
//   Eigen::VectorXd &temp = workspace.temp[thread_id];
//   temp.setZero();
...
//                                  &diff, Eigen::VectorXd &rhs_grad, double
//                                  lambda, QP_pass_workspace &workspace, int
//                                  thread_id, int cost_dim, double dt, int
//                                  batch_id, int seq_len, int time) {
//   Eigen::VectorXd &temp = workspace.temp[thread_id];
//   temp.setZero();
...
//     int batch_id, int seq_len, int cost_dim, int eq_dim, int tool_id, double
//     dt, Eigen::Tensor<double, 3, Eigen::RowMajor> grad_output) {
...
//   grad_e += (workspace.lambda_L1 * sign_e_scaled.array() *
//   w.array()).matrix(); Eigen::MatrixXd Adj =
//   workspace.last_T[batch_id].inverse().toActionMatrix(); Eigen::MatrixXd Jlog
//   = pinocchio::Jlog6(workspace.last_T[batch_id]); Eigen::MatrixXd J_frame(6,
//   model.nv); J_frame.setZero(); pinocchio::computeFrameJacobian(model, data,
//   workspace.last_q[batch_id],
...
//     double *q_ptr = workspace.positions_.data() +
//                     batch_id * (seq_len + 1) * cost_dim + (time + 1) *
//                     cost_dim;
...
//           cost_dim);
//       compute_direct_grad_path(grad_vec_local, workspace, thread_id,
//       cost_dim,
//                                dt);
//       update_indirect_grad_path_1(grad_vec_local, model, diff, rhs_grad,
//       lambda,
//                                   workspace, thread_id, cost_dim, dt);
//       update_indirect_grad_path_2(grad_vec_local, model, diff, rhs_grad,
//       lambda,
//                                   workspace, thread_id, cost_dim, dt,
...
//       workspace.steps_per_batch[batch_id];
//     }
//   }
// }

// void backward_pass(QP_pass_workspace &workspace, pinocchio::Model &model,
//                    const Eigen::Tensor<double, 3, Eigen::RowMajor>
//                    &grad_output, int num_thread, int batch_size) {
//   int cost_dim = model.nq;
